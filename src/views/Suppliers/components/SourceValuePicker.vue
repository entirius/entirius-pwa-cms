<template>
  <div class="source-value-picker" v-out="() => closeDropdown()">
    ...
    >
      ...
      >
        {{ $t("suppliers.mappings.category.source_value_picker_no_source_field") }}
      </div>
      <div
        v-else-if="error"
      ...
      >
        {{
          values.length
            ...
            : $t("suppliers.mappings.category.source_value_picker_empty")
        }}
      </div>
      <ul v-else class="source-value-picker__list">
        ...
        >
          ...
          <span class="source-value-picker__count t-basic-500 fs-200">
            {{ item.count }}
            {{ $t("suppliers.mappings.category.source_value_picker_count_suffix") }}
          </span>
        </li>
      </ul>
    </div>
...
<script>
import { GET_DataValues } from "@/api/suppliers/api";
