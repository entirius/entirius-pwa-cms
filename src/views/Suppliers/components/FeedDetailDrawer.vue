    ...
    <div v-if="feed" class="feed-detail">
      ...
      <section class="feed-detail__section">
        <h3 class="feed-detail__heading">
          {{ $t("suppliers.feeds.detail.sync_section") }}
        </h3>
        <dl class="feed-detail__grid">
          <dt>{{ $t("suppliers.feeds.col.schedule") }}</dt>
          <dd>{{ feed.schedule_cron || dash }}</dd>
          <dt>{{ $t("suppliers.feeds.col.last_sync") }}</dt>
          <dd>{{ formatDate(feed.last_sync_at) }}</dd>
          <dt>{{ $t("suppliers.feeds.detail.last_run_id") }}</dt>
          <dd class="feed-detail__mono">{{ feed.last_run_id || dash }}</dd>
          <dt>{{ $t("suppliers.feeds.detail.created_at") }}</dt>
          <dd>{{ formatDate(feed.created_at) }}</dd>
          <dt>{{ $t("suppliers.feeds.detail.modified_at") }}</dt>
      ...
      >
        <h3 class="feed-detail__heading">
          {{ $t("suppliers.feeds.detail.overrides_section") }}
        </h3>
        <dl class="feed-detail__grid">
          <template v-if="feed.feature_set_idx">
            <dt>{{ $t("suppliers.feeds.detail.feature_set_idx") }}</dt>
            <dd>{{ feed.feature_set_idx }}</dd>
          </template>
          <template v-if="feed.language_id != null">
            <dt>{{ $t("suppliers.feeds.detail.language_id") }}</dt>
            <dd>{{ feed.language_id }}</dd>
          </template>
          <template v-if="feed.currency_id != null">
            <dt>{{ $t("suppliers.feeds.detail.currency_id") }}</dt>
      ...
      <section class="feed-detail__section">
        <h3 class="feed-detail__heading">
          {{ $t("suppliers.feeds.detail.config_section") }}
        </h3>
        <p class="fs-200 t-basic-500 mb-200">
          {{ $t("suppliers.feeds.detail.config_hint") }}
        </p>
      ...
      <section class="feed-detail__section">
        <div class="flex ai-ct jc-sb mb-200">
          <h3 class="feed-detail__heading m-0">
            {{ $t("suppliers.feeds.detail.preview_section") }}
          </h3>
          <button
            class="feed-detail__refresh-btn"
            :disabled="previewBusy"
            :title="$t('suppliers.feeds.detail.preview_refresh')"
            @click="loadPreview"
          >
          ...
          >
            ...
            <div class="flex ai-ct gap-300 flex-wrap fs-200 t-basic-500">
              ...
              <a
                ...
                class="t-support-400"
              >
                {{ $t("suppliers.feeds.detail.preview_open_source") }}
              </a>
            </div>
          </div>
        </div>
...
<script>
import { POST_FeedTest } from "@/api/suppliers/api";
...
export default {
  ...
  computed: {
    title() {
      return this.$t("suppliers.feeds.detail.title", {
        idx: this.feed?.idx || "",
      });
    },
    feedConfigPretty() {
