<template>
  <div class="feeds-tab p-300 ovy-auto h-100">
    <div class="flex ai-ct jc-sb mb-300">
      <h2 class="fs-400 fw-600">{{ $t("suppliers.tabs.feeds") }}</h2>
      <p class="fs-200 t-basic-500 m-0">
        {{ $t("suppliers.feeds.readonly_hint") }}
      </p>
    </div>
    ...
    <DataTable
      ...
      :rows="feeds"
      row-key="idx"
      :empty-text="$t('suppliers.feeds.empty')"
      ...
      <template #cell-actions="{ row }">
        <div class="flex ai-ct gap-100" @click.stop>
          <button
            ...
            :title="$t('suppliers.feeds.trigger_button')"
    ...
    >
      <template #header>
        <h2>{{ $t("suppliers.feeds.trigger_title") }}</h2>
      </template>
      <template #description>
        <p>
          {{
            $t("suppliers.feeds.trigger_body", {
...
import { GET_Feeds, POST_FeedTrigger } from "@/api/suppliers/api";
...
export default {
  ...
  computed: {
    columns() {
      return [
        { key: "idx", label: this.$t("suppliers.col.idx"), width: "1fr" },
        {
          key: "connector_kind",
          label: this.$t("suppliers.feeds.col.connector"),
          width: "140px",
        },
        {
          key: "last_sync_at",
          label: this.$t("suppliers.feeds.col.last_sync"),
          width: "1.5fr",
        },
        {
          key: "status_combined",
  ...
  methods: {
    ...
    async executeTrigger() {
      ...
      try {
        ...
        this.notify.spawnNotification({
          type: "positive",
          msg: this.$t("suppliers.feeds.toast.triggered", {
            run_id: data.run_id || "—",
          }),
        });
