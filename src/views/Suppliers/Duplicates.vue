  ...
  <div class="p-500 fs-300 t-basic-800 h-100 ov-h">
    ...
    >
      <div class="flex ai-ct jc-sb mb-400 gap-300">
        <h1 class="fs-700 fw-600">{{ $t("suppliers.duplicates.title") }}</h1>
      </div>
      <p class="fs-300 t-basic-700 mb-400">
        {{ $t("suppliers.duplicates.subtitle") }}
      </p>
      ...
      <EmptyState
        ...
        :title="$t('suppliers.duplicates.empty_title')"
        :message="$t('suppliers.duplicates.empty_message')"
        icon="copy"
      />
      ...
      >
        <div class="flex ai-ct jc-sb mb-300 gap-200">
          <div>
            <p class="fs-200 t-basic-500 fw-600">
              {{ $t("suppliers.duplicates.col.ean") }}
            </p>
        ...
        <table class="duplicates-table">
          <thead>
            <tr>
              <th>{{ $t("suppliers.duplicates.col.sku") }}</th>
              <th>{{ $t("suppliers.duplicates.col.weight") }}</th>
              <th>{{ $t("suppliers.duplicates.col.suppliers") }}</th>
              <th>{{ $t("suppliers.duplicates.col.actions") }}</th>
            </tr>
          </thead>
          <tbody>
            <tr v-for="rp in group.realproducts" :key="rp.sku">
              ...
              <td>
                <div class="flex ai-ct flex-wrap gap-100">
                  ...
                  >
                    {{ $t("suppliers.duplicates.action.merge_to", { sku: other.sku }) }}
                  </button>
                </div>
              </td>
            </tr>
...
<script>
import { GET_Duplicates } from "@/api/suppliers/api";
import MergeConfirmationModal from "@/views/Suppliers/components/MergeConfirmationModal.vue";
...
export default {
  ...
  methods: {
    ...
    suggestionLabel(suggestion) {
      return this.$t(`suppliers.duplicates.suggestion.${suggestion}`);
    },
    ...
    onMergeConfirmed(payload) {
      ...
      notify.spawnNotification({
        type: "positive",
        msg: this.$t("suppliers.duplicates.merge_modal.success", {
