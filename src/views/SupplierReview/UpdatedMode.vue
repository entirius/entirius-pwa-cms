<template>
  <div class="updated-mode">
    <div class="updated-mode__header flex ai-ct jc-sb mb-200 flex-wrap gap-200">
      <div>
        <h3 class="fs-300 fw-600 m-0">
          {{ $t("supplier_review.updated.title") }}
        </h3>
        <p class="fs-200 t-basic-500 mt-100 mb-0">
          {{ $t("supplier_review.updated.help") }}
        </p>
      </div>
      <div class="flex ai-ct gap-200">
        ...
        >
          {{ $t("supplier_review.updated.counter", {
    ...
    <DataTable
      ...
      selectable
      multi-select
      :empty-text="$t('supplier_review.updated.empty')"
      @select="onSelect"
    >
      <template #cell-supplier_name="{ row }">
        <span class="fs-200">{{ row.supplier_name || row.supplier_idx || "—" }}</span>
      </template>
    ...
    <EmptyState
      ...
      icon="bell-slash"
      :title="$t('supplier_review.updated.empty')"
      :message="$t('supplier_review.updated.empty_message')"
    />
  </div>
</template>
...
import { GET_SupplierProducts } from "@/api/suppliers/api";
...
export default {
  ...
  setup() {
    return { notify: useNotifyStore(), bulk: useSupplierBulkActions() };
  },
  data() {
    return {
      rows: [],
  ...
  computed: {
    columns() {
      return [
        ...
        {
          key: "last_change",
          label: this.$t("supplier_review.updated.col.last_change"),
          width: "150px",
        },
      ];
    },
    ...
    perSupplierCounts() {
      ...
      for (const row of this.visibleRows) {
        const idx = row.supplier_idx;
        if (!idx) continue;
        const entry = acc.get(idx) || { idx, name: row.supplier_name || idx, count: 0 };
        entry.count += 1;
        acc.set(idx, entry);
      }
  ...
  methods: {
    ...
    reportBulkResult(actionKey, successCount, failedCount) {
      const keyMap = {
        force_repush: {
          ok: "suppliers.products.bulk.toast.force_repush_succeeded",
          partial: "suppliers.products.bulk.toast.force_repush_partial",
        },
        acknowledge: {
          ok: "suppliers.products.bulk.toast.acknowledge_succeeded",
          partial: "suppliers.products.bulk.toast.acknowledge_partial",
        },
      };
