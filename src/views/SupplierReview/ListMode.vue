<template>
  <div class="list-mode">
    ...
    >
      ...
      >
        {{
          $t("supplier_review.list.selected_count", { count: selected.length })
        }}
      </span>
      <button
        ...
        :title="
          hasMonitoringSelected
            ? $t('suppliers.products.monitoring_tooltip')
            : ''
        "
      ...
      >
        ...
        {{ $t("supplier_review.list.bulk_approve") }}
      </button>
      <button
      ...
      >
        ...
        {{ $t("supplier_review.list.bulk_reject") }}
      </button>
      <button
      ...
      >
        ...
        {{ $t("supplier_review.list.bulk_requeue") }}
      </button>
      <button
      ...
      >
        ...
        {{ $t("supplier_review.list.push_approved") }}
      </button>
    </div>
    ...
    <DataTable
      ...
      :sortable="true"
      row-key="id"
      :empty-text="$t('supplier_review.empty_state_title')"
    ...
    >
      <template #header>
        <h2>{{ $t("supplier_review.list.confirm_title") }}</h2>
      </template>
      <template #description>
        <p>
          {{
            $t(`supplier_review.list.confirm_${pendingAction}`, {
  ...
  computed: {
    columns() {
      return [
        ...
        {
          key: "supplier_idx",
          label: this.$t("supplier_review.list.col.supplier"),
          width: "120px",
          sortable: false,
        },
        {
    ...
    hasMonitoringSelected() {
      // Approve/push are PIM-bound; the backend refuses them for monitoring suppliers.
  ...
  methods: {
    ...
    async fetchRows() {
      ...
      try {
        ...
        if (this.filters.supplier && this.filters.supplier !== "__all") {
          params.supplier_idx = this.filters.supplier;
        }
    ...
    async bulkExecute() {
      ...
      try {
        ...
        this.notify.spawnNotification({
          type: "positive",
          msg: this.$t("supplier_review.list.toast.bulk_success", {
    ...
    async bulkPush() {
      ...
      try {
        ...
        this.notify.spawnNotification({
          type: "positive",
          msg: this.$t("supplier_review.list.toast.push_dispatched"),
        });
        this.fetchRows();
      } catch (err) {
        this.notify.spawnNotification({
    ...
    canApprove(p) {
      return (
        !this.isDetailMonitoring(p) && ["new", "queued"].includes(p?.status)
      );
    },
    canSkip(p) {
